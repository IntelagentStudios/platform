...
RUN npx turbo run db:generate --filter=@intelagent/admin-portal --filter=@intelagent/customer-portal

# Build all applications
RUN npx turbo run build --filter=@intelagent/admin-portal
...
# Copy package files first (without workspace packages)
COPY package.json ./
COPY package-lock.json ./

# Remove postinstall script temporarily for production install
