...
export class BlockchainConnectorSkill extends BaseSkill {
  metadata = {
    id: 'blockchain_connector',
    name: 'Blockchain Connector',
    description: 'Connect to blockchains',
    category: SkillCategory.BLOCKCHAIN,
    version: '2.0.0',
    author: 'Intelagent',
    tags: ["blockchain","artificial-intelligence","blockchain","web3","blockchain-connector"]
  };
  ...
  async execute(params: SkillParams): Promise<SkillResult> {
    try {
      ...
      const result = await this.processBlockchainConnector(params);
  ...
  private async processBlockchainConnector(params: SkillParams): Promise<any> {
    ...
    console.log(`[BlockchainConnectorSkill] Processing ${action} for license ${licenseKey}, task ${taskId}`);
