...
export class ComplianceCheckerSkill extends BaseSkill {
  metadata = {
    ...
    version: '2.0.0',
    author: 'Intelagent',
    tags: ["legal","legal","compliance","compliance-checker"]
  };
  ...
  async execute(params: SkillParams): Promise<SkillResult> {
    try {
      ...
      const result = await this.processComplianceChecker(params);
  ...
  private async processComplianceChecker(params: SkillParams): Promise<any> {
    ...
    console.log(`[ComplianceCheckerSkill] Processing ${action} for license ${licenseKey}, task ${taskId}`);
