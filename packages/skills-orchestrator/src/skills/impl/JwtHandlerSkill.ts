...
export class JwtHandlerSkill extends BaseSkill {
  metadata = {
    id: 'jwt_handler',
    name: 'JWT Handler',
    description: 'Internal jwt handler implementation',
  ...
  async execute(params: SkillParams): Promise<SkillResult> {
    try {
      ...
      console.log(`[JWT Handler] Executing for license ${licenseKey}, task ${taskId}`);
  ...
  private async queueMessage(message: any): Promise<void> {
    console.log('[JWT Handler] Message queued:', message);
  }
  ...
  private async analyzeJWTHandler(data: any): Promise<any> {
    return {
      result: 'analyzed',
      confidence: 0.95,
      data
  ...
  private async scheduleTask(id: string, task: any, schedule: any): Promise<void> {
    console.log(`[JWT Handler] Task scheduled: ${id}`);
  }
  
  private async executeTask(id: string, task: any): Promise<void> {
    console.log(`[JWT Handler] Task executed: ${id}`);
  }
  ...
  private async processJWTHandler(entity: any): Promise<any> {
