...
export class ListingManagerSkill extends BaseSkill {
  metadata = {
    id: 'listing_manager',
    ...
    version: '2.0.0',
    author: 'Intelagent',
    tags: ["realestate","listing-manager"]
  };
  ...
  protected async executeImpl(params: SkillParams): Promise<SkillResult> {
    try {
      ...
      const result = await this.processListingManager(params);
  ...
  private async processListingManager(params: SkillParams): Promise<any> {
    ...
    console.log(`[ListingManagerSkill] Processing ${action} for license ${licenseKey}, task ${taskId}`);
