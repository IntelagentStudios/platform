...
export class MetadataTaggerSkill extends BaseSkill {
  metadata = {
    id: 'metadata_tagger',
    name: 'Metadata Tagger',
    description: 'Tag media metadata',
    category: SkillCategory.MEDIA,
    version: '2.0.0',
    author: 'Intelagent',
    tags: ["media","metadata-tagger"]
  };
  ...
  async execute(params: SkillParams): Promise<SkillResult> {
    try {
      ...
      const result = await this.processMetadataTagger(params);
  ...
  private async processMetadataTagger(params: SkillParams): Promise<any> {
    ...
    console.log(`[MetadataTaggerSkill] Processing ${action} for license ${licenseKey}, task ${taskId}`);
