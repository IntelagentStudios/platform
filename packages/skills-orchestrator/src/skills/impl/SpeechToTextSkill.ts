...
export class SpeechToTextSkill extends BaseSkill {
  metadata = {
    id: 'speech_to_text',
    name: 'Speech To Text',
  ...
  async execute(params: SkillParams): Promise<SkillResult> {
    try {
      ...
      // Process SpeechToText operation
      const result = await this.processSpeechToText(params, core);
  ...
  private async processSpeechToText(params: SkillParams, core: SkillCore): Promise<any> {
    // Skill-specific processing logic
    const { action = 'default' } = params;
    
    switch (action) {
      case 'default':
        return this.handleDefaultSpeechToText(params, core);
      default:
        return this.handleDefaultSpeechToText(params, core);
    }
  }
  
  private async handleDefaultSpeechToText(params: SkillParams, core: SkillCore): Promise<any> {
    // Default implementation
    await this.delay(Math.random() * 200 + 100);
    
    return {
      action: 'processed',
      skillName: 'SpeechToText',
